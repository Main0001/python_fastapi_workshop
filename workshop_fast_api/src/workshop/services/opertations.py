...
from ..models.operations import OpetationKind, OperationCreate, OperationUpdate


class OpertationsService:
    ...
    def _get(self, user_id: int, operation_id: int) -> tables.Operation:
        operation = (
            ...
            filter_by(id=operation_id, user_id=user_id).first()
        )
        if not operation:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
        return operation

    def get_list(self, user_id: int, kind: Optional[OpetationKind] = None) -> List[tables.Operation]:
        query = (
            self.session.query(tables.Operation)
